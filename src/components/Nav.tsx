    ...
    <AnimatePresence>
      {isElementTouchTop && (
        <motion.nav
          ...
          className="fixed z-20 right-5 bottom-10  py-4 max-w-[700px] bg-white shadow-xl rounded-full px-7  flex gap-7 items-center transition-all duration-300"
        >
          <Link href={"/"}>
            <svg
              className={`nav-icon w-[30px] h-[30px] ${getIconColor("/")}`}
              viewBox="0 0 23 24"
              fill="none"
          ...
          <button onClick={openSearch}>
            <svg
              className={`nav-icon w-[30px] h-[30px] fill-slate-600`}
              viewBox="0 0 25 25"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
