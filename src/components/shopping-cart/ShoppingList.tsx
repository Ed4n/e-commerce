...
type ShoppingListProps = {
    productsToShop: ProductWithQuantity[]
}

const ShoppingList: React.FC<ShoppingListProps> = ({ productsToShop }) => {
        ...
        <div>
            {
                productsToShop.length <= 0 ? (
                    <div className="text-gray-300 top-0 flex justify-center items-center w-full h-[40vh] gap-4 flex-col text-xl">
                        <svg className="nav-icon first-line:w-[40px] h-[40px] fill-gray-300" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                        ...
                        </svg> Add Some Products
                    </div>

                ) : (
                    productsToShop.map((item: ProductWithQuantity) => (
                        <ShoppingProduct key={item._id} product={item} />
                    ))
                )
